...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      category: ["축구", "풋살", "농구", "야구", "볼링", "골프", "테니스", "배드민턴", "헬스", "필라테스", "격투기", "수영"],
    ...
    const checkDate = () => {
      if (state.closeDt > state.meetDt) {
        Swal.fire({
          icon: "error",
          title: "실패...",
          text: "운동일이나 이전 날짜를 선택해주세요.",
        });
      }
    };
    return { state, chekingCategory, chekingGender, checkingAge, checkingSido, checkingGungu, checkingTime, mateCreate, checkDate };
  },
};
</script>
