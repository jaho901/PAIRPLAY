  ...
  <div 
    ...
    >
      ...
      <div class="col-2 my-4 d-flex justify-content-around" style="flex-direction: column;">
        <button class="btn-mate accept" @click="mateToCancle(data.id)">
          신청 취소
        </button>
      </div>
    </div>
    ...
    <div>
      <ul class="pagination mt-5">
        ...
        <!-- aria-current="page" -->
        <li v-for="idx in state.profileMateListToTotalPage" :key="idx" class="page-item-to">
...
export default {
  ...
  setup () {
    ...
    onMounted (async () => {
      var activeBtn = document.getElementsByClassName("page-item-to")[state.pageTo-1]
      activeBtn.classList.add("active")
    })

    const changePage = async function (event) {
      for (var i=0; i < state.profileMateListToTotalPage; i++) {
        var Btn = document.getElementsByClassName("page-item-to")[i]
        Btn.classList.remove("active")
      }
      var activeBtn = document.getElementsByClassName("page-item-to")[Number(event.target.textContent)-1]
      activeBtn.classList.add("active")
      state.pageTo = Number(event.target.textContent)
      await store.dispatch("root/profileMateListTo", {
        "page": Number(event.target.textContent)-1,
        "size": 3,
      })
    }

    const mateToCancle = async function (id) {
      await store.dispatch("root/profileMateToCancle", {
        'id': id,
        'page': state.pageTo-1,
      })
    }
    ...
    return { state, onMounted, changePage, mateToCancle, moveToProfile }
  }
}
</script>
